		...
		<table>
			...
			<tbody>
				<?php if(!empty($list)): if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$rental): $mod = ($i % 2 );++$i;?><tr>
						...
						<td><a href="<?php echo U('index?pid='.$rental['id']);?>"><?php echo ($rental["title"]); ?></td>
                        <td><?php echo ($rental['type']==0?'出租':'销售'); ?></td>
						<td><?php echo ($rental["price"]); ?></td>
						<td><?php echo ($rental['unit']==0?'元/月':'万元'); ?></td>
						<td><?php echo date('Y-m-d G:i:s',$rental['start_time']);?></td>
						<td><?php echo ($rental["end_time"]); ?></td>
						<td><?php echo ($rental['status']==0?'禁用':'启用'); ?></td>
						<td>
